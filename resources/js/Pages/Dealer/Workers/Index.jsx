...
import Create from "@/Pages/Dealer/Workers/WorkerCreate.jsx";
import Update from "@/Pages/Dealer/Workers/WorkerUpdate.jsx";
...
export default function Workers({auth, workersAll, csrf_token, dealer}) {
                        ...
                        <Button size={"small"}
                                onClick={(event) => {
                                    confirmPopup({
                                        ...
                                        accept: () => {
                                            let url = route('dealer.workers.multipleDestroy');
                                            fetch(url, {
                                                method: 'POST',
                                                headers: {
    ...
    const confirmPopupForDealerDelete = (event, itemId, callback = () => {
        ...
        const deleteFunction = () => {
            fetch(route('dealer.workers.destroy', itemId), {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrf_token
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        ...
                        <Dialog header="Yeni Çalışan Ekle" style={{width: '50vw'}}
                            ...
                            <Create dealerId={dealer.id} onHide={() => setAddModal(false)} csrf_token={csrf_token}
                                    toast={toast} setWorkers={setWorkers} addModal={addModal}
                        ...
                        <Dialog header="Çalışanı Düzenle" style={{width: '50vw'}}
                            ...
                            <Update updateModal={updateModal} worker={updateWorker} csrf_token={csrf_token}
                                    toast={toast} onHide={() => setUpdateModal(false)} setWorkers={setWorkers}
                                ...
                                <div className="flex flex-col gap-y-2">
                                    ...
                                    <div className={"text-center mt-3"}>
                                        <ButtonGroup>
                                            ...
                                            <Button label="Hizmet Kayıtları" icon="pi pi-file" severity={"help"}
                                                    onClick={() => {
                                                        // TODO: Hizmet Kayıtları
                                                    }}/>
                                            <Button label="Düzenle" icon="pi pi-pencil" severity={"warning"}
                                                    onClick={(e) => {
                                                        setViewModal(false);
                                                        setViewWorker({});
                                                        updateWorkerPrepare(viewWorker);
                                                    }}/>
                                            <Button label="Sil" icon="pi pi-trash" severity={"danger"}
                                                    onClick={(event) => {
