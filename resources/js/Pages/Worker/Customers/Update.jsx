...
export default function Update({csrf_token, setUsers, user, visible, setVisible}) {
    const toast = useRef(null);
    const [loading, setLoading] = useState(false);
    
    const notificationSettings = useMemo(() => {
        if (user?.notification_settings) {
            return Object.entries(JSON.parse(user.notification_settings))
    ...
    return (
        <>
            <Toast ref={toast}/>
            <Dialog
                header={`${isCompany ? 'Kurumsal' : 'Bireysel'} Müşteri Düzenle`}
                visible={visible}
                style={{width: '50vw'}}
                ...
                <BlockUI blocked={loading}>
                    <form onSubmit={formik.handleSubmit} className="flex flex-col gap-4">
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="field">
                                <label htmlFor="player_id" className="font-bold block mb-2">
                                    Device ID
                                </label>
                                <InputText
                                    id="player_id"
