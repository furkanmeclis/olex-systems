    ...
    const PageContent = () => {
        ...
        const getDealers = () => {
            ...
            fetch(route('super.dealers.listAll'), {
                ...
                console.log(err);
                toast.current.show({
                    severity: 'error', summary: 'Hata', detail: 'Bayiler getirilirken bir hata oluştu.'
                });
            }).finally(() => {
                setLoading(false);
            });
        ...
        const getProducts = () => {
            ...
            fetch(route('super.products.listAll'), {
                ...
                console.log(err);
                toast.current.show({
                    severity: 'error', summary: 'Hata', detail: 'Ürünler getirilirken bir hata oluştu.'
                });
            }).finally(() => {
                setLoading(false);
            });
            ...
            return (<>
                <Toolbar
                    ...
                    center={() => <><p className={"flex flex-col items-center justify-center"}>
                        ...
                        {selectedRows.length > 0 && (<span>{selectedRows.length} sipariş seçildi.</span>)}</p>
                    </>}
                />
            </>);
        };
        ...
        return <>
            ...
            <DataTable editMode="cell" dragSelection value={records} removableSort paginator
                ...
                {selectedColumns.includes('dealer') &&
                    <Column field="dealer.name" filter filterPlaceholder={"Bayi Adına Göre Filtreleyin"}
                            align={"center"}
                ...
                {selectedColumns.includes('user') &&
                    <Column field="user.name" filter filterPlaceholder={"Yetkiliye Göre Filtreleyin"} sortable
                            header="Yetkili"/>}
                {selectedColumns.includes('status') && <Column
                    ...
                    filter
                    filterElement={(options) => <Dropdown value={options.value}
                ...
                {selectedColumns.includes('products_count') &&
                    <Column field="products_count" body={(rowData) => <Button link onClick={() => {
                        if (rowData.products_count === 0) {
                            toast.current.show({
                                severity: 'info', summary: 'Bilgi', detail: 'Siparişte ürün bulunmamaktadır.'
                            });
                        } else {
