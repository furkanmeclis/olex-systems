                    ...
                    <Button size={"small"}
                                    ...
                                    accept: () => {
                                        ...
                                        fetch(url, {
                                            method: 'POST', headers: {
                                                ...
                                                workerIds: selectedWorkers.map(worker => worker.id), dealerId: dealer.id
                                            })
                                        }).then(async response => {
                     ...
                     center={() => <>
                         <div className={"flex flex-col items-center justify-center"}>
                             ...
                             {selectedWorkers.length > 0 && (<span>{selectedWorkers.length} çalışan seçildi.</span>)}
                         </div>
                     </>}
            />
        </>);
    ...
    return (<>
        ...
        <DataTable dragSelection value={workers} removableSort paginator
            ...
            {selectedColumns.includes('name') &&
                <Column field="name" filter filterPlaceholder={"Ada Göre Filtreleyin"} sortable header="Çalışan Adı"/>}
            {selectedColumns.includes('email') &&
                <Column field="email" filter filterPlaceholder={"Emaile Göre Filtreleyin"} sortable header="Email"/>}
            {selectedColumns.includes('phone') &&
                <Column field="phone" filter filterPlaceholder={"Telefona Göre Filtreleyin"} sortable
                        header="Telefon No"/>}
            {selectedColumns.includes('avatar') && <Column field="image" header="Avatar" body={(user) => {
                return <Avatar image={user.avatar} shape="circle" size={"large"}/>
            }}/>}
            {selectedColumns.includes('active') && <Column field="active" header="Durumu" dataType="boolean"
            ...
            {selectedColumns.includes('updated_at') && <Column field="updated_at" sortable header="Güncellenme Tarihi"
        ...
        <Dialog header="Yeni Çalışan Ekle" style={{width: '50vw'}} breakpoints={{'960px': '75vw', '641px': '100vw'}}
                onHide={() => setAddModal(false)} maximizable visible={addModal} footer={addModalFooter}>
            <Create dealerId={dealer.id} onHide={() => setAddModal(false)} csrf_token={csrf_token} toast={toast}
                    setWorkers={setWorkers} addModal={addModal} setAddModalFooter={setAddModalFooter}/>
        </Dialog>
        <Dialog header="Çalışanı Düzenle" style={{width: '50vw'}} breakpoints={{'960px': '75vw', '641px': '100vw'}}
                onHide={() => setUpdateModal(false)} maximizable visible={updateModal} footer={updateModalFooter}>
            <Update updateModal={updateModal} worker={updateWorker} csrf_token={csrf_token} toast={toast}
                    onHide={() => setUpdateModal(false)} setWorkers={setWorkers}
                    setUpdateModalFooter={setUpdateModalFooter}/>
        </Dialog>
        <Dialog header="Çalışanı Görüntüle" style={{width: '40vw'}}
                breakpoints={{'960px': '75vw', '641px': '100vw'}} onHide={() => setViewModal(false)} maximizable
                visible={viewModal}>
