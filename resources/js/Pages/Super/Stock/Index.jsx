...
export default function Index({auth, recordsAll, csrf_token, statuses}) {
    ...
    const quantityEditor = (option) => {
        return <InputNumber value={option.value} onValueChange={(e) => option.editorCallback(e.value)} showButtons
                            min={0}/>
    }
    const renderHeader = () => {
        return (<>
    ...
    const onRowEditComplete = async (e) => {
        const record = {
            id: e.newData.id, quantity: e.newData.quantity, status: e.newData.status
        }
        setLoading(true);
    ...
    return (<AuthenticatedLayout
        user={auth.user}
        header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Stok
            Kayıtları</h2>}
    >
            ...
            <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <DataTable dragSelection value={records} removableSort paginator
                               ...
                               filterDisplay={"row"}
                               filters={{
                                   "dealer.name": {value: "", matchMode: "contains"},
                                   "product.name": {value: "", matchMode: "contains"},
                        ...
                        <Column field="dealer.name" filter filterPlaceholder={"Bayi Adına Göre"} sortable header="Bayi Adı"/>
                        <Column field="product.name" filter filterPlaceholder={"Ürün Adına Göre"} sortable header="Ürün Adı"/>
                        ...
                        <Column field="updated_at" sortable header="Son Değişiklik Tarihi"
                                body={(rowData) => new Date(rowData.updated_at).toLocaleString()}/>
                        <Column sortable header="Ürün Kodları" body={(rowData) => <Button icon={"pi pi-qrcode"}
                                                                                          tooltip={"Ürün Kodlarını Görüntüle"}
                                                                                          onClick={() => {
        ...
        <Dialog header="Ürünler" style={{width: '50vw'}} breakpoints={{'960px': '75vw', '641px': '100vw'}}
            ...
            return <><Button label="Kapat" icon={"pi pi-times"} onClick={() => setProductsModal(false)}
                             severity={"warning"}
                             size={"small"}/>
                <Button label={"Kaydet"} icon={"pi pi-cloud-download"} size={"small"} severity={"success"}
                        onClick={() => {
                            if (productsModalData.codes.length === 0) {
                                toast.current.show({
                                    severity: 'warn', summary: 'Hata', detail: "Ürün Kodu Bulunamadı"
                                });
                                return;
                            } else {
                                let _txtData = "";
